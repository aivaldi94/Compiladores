			   ...
			   fun aux4 ([] venv pos) = []
			     | aux4 ({body = b, name=nom, ... } :: rvs) (venv :: venvs) (p :: poss) =
			     	 let
						...
						val lvl = case tabBusca (nom,venv) of
								NONE => error("Agregando argumentos de una función que no está en el entorno",p)
							  | SOME (VIntro _) => error("No deberia pasar. No es funcion", p) 
							  | SOME (Var _) =>	error("No deberia pasar. No es funcion", p)
							  | SOME (Func {level = l, ...}) => l
						val f = transExp (venv , tenv, lvl) (*Deberìa ser una función que toma una exp*)
						val elem = #ty (f b)						
						in elem :: (aux4 rvs venvs poss) end
			   val tipos = aux4 (List.map (fn (fs,_) => fs) xs) venvs listPos
			   (* aux5 :  *)
