...
*)
		...
		| trdec (venv,tenv) (FunctionDec xs) =  
			let
			   ...
			   val l1 = ListPair.zip(List.map (fn (fs,_) => fs) xs, venvs) : (recfun * venv) list
			   val l2 = ListPair.zip (l1,listPos) : ((recfun * venv) * int) list
			   val l3 = List.map (fn ((a,b),c) => (a,b,c)) : (recfun * venv * int) list
			   val tipos = aux4 l3
			   (* aux5 :  *)
